...
export const Layout: React.FC<LayoutProps> = ({ 
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#f1f5f9] text-slate-800">
      ...
      {showKeyPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[1000] flex items-center justify-center p-4">
           <div className="bg-white rounded-[3.5rem] p-12 w-full max-w-md shadow-[0_0_120px_rgba(0,0,0,0.6)] border-8 border-blue-50 animate-in zoom-in-95">
              ...
              <p className="mt-8 text-center text-[9px] text-slate-300 font-bold uppercase italic tracking-[0.2em] leading-relaxed">
                Dữ liệu chỉ lưu trong sessionStorage.<br/>Không lưu trữ trên máy chủ hệ thống.
              </p>
           </div>
        </div>
      )}
      ...
      <footer className="bg-white border-t border-slate-200 px-8 py-2.5 text-[11px] text-slate-500 font-medium flex justify-between items-center shadow-inner">
        <div className="flex items-center space-x-4">
          ...
          <span className="font-bold text-slate-400 uppercase text-[10px]">{profile.licenseInfo}</span>
          <span className="ml-5 bg-indigo-50 text-indigo-600 px-4 py-1.5 rounded-full text-[9px] font-black uppercase shadow-sm">Sử dụng: {usageCount} (Bản miễn phí)</span>
        </div>
