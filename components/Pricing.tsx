  ...
  return (
    <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-lg flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
      <div className="bg-white rounded-[2rem] shadow-[0_0_60px_rgba(0,0,0,0.3)] w-full max-w-6xl overflow-hidden animate-in zoom-in-95 flex flex-col relative border border-slate-200 max-h-[95vh]">
        ...
        {/* Modal Header */}
        <div className="bg-indigo-950 text-white px-8 py-5 flex items-center justify-between border-b border-white/5 shadow-md">
        ...
        <div className="p-10 overflow-y-auto flex flex-col items-center flex-1 bg-slate-50/50 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full max-w-6xl mb-14">
            {SUBSCRIPTION_PLANS.map((plan) => (
              <div 
                key={plan.id} 
                className={`bg-white rounded-[2.5rem] border-2 ${selectedPlan?.id === plan.id ? 'border-emerald-500 shadow-2xl shadow-emerald-100 scale-105' : 'border-slate-100 shadow-xl'} p-8 hover:border-emerald-200 transition-all duration-500 group cursor-pointer relative flex flex-col items-center`}
                onClick={() => setSelectedPlan(plan)}
              >
                {selectedPlan?.id === plan.id && (
                   <div className="absolute -top-4 bg-emerald-500 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase shadow-lg animate-bounce">GÓI ĐANG CHỌN</div>
                )}
                ...
                >
                  {selectedPlan?.id === plan.id ? 'XÁC NHẬN KÍCH HOẠT' : 'CHỌN GÓI NÀY'}
                </button>
              </div>
            ))}
          </div>

          <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-10 bg-white border border-slate-200 rounded-[3rem] p-12 shadow-2xl relative overflow-hidden animate-in slide-in-from-bottom-10 duration-500">
              ...
              <div className="space-y-6">
                ...
                <div className="p-6 bg-amber-50 rounded-3xl border border-amber-200 shadow-inner">
                  <p className="text-[13px] font-bold text-amber-900 leading-relaxed italic">
                    💡 Lưu ý: <span className="font-black text-red-600 underline decoration-red-300 decoration-2">{BANK_INFO.name} + {selectedPlan?.name || "Kích hoạt"}</span>
                    <br/>
                    <span className="text-[11px] mt-2 block text-amber-700 font-medium">Lưu ý: Hệ thống sẽ tự động kích hoạt dựa trên ID Máy của bạn ngay sau khi nhận được thông báo.</span>
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center justify-center bg-slate-50 rounded-[2.5rem] border border-slate-200 shadow-inner p-10">
              <div className="bg-white p-6 rounded-[2rem] shadow-2xl mb-8 transform hover:scale-105 transition duration-500 border border-slate-100 relative group">
                <img 
                  src={`https://api.qrserver.com/v1/create-qr-code/?size=280x280&data=MB|${BANK_INFO.account}|KhaiPham`} 
